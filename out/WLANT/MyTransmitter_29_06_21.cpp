...
//#include "out/WLANT/times.cpp"

#define BUF_SIZE 256
using namespace std;

std::ofstream log_file("log_file_Receiver.txt", std::ios_base::out);
long msec = 0;
...
static std::vector<std::complex<float>> MyTransmitter(char* param, int xx, unsigned int *num_of_samples2, char buf[BUF_SIZE]){ // было
    time_t sec;
    sec = time (NULL);
...
static std::vector<std::complex<float>> simulate1(double SNR, unsigned packet_size, unsigned data_rate, int channel_type, int use_ldpc, int packet_count, unsigned int *num_of_samples2, const char buf[BUF_SIZE])
{
 ...
 /*   unsigned int i;
    ...
    //std::complex<float> time_signal[10000]; // было так, но выделили память теперь динамически
...
*/
    for ( unsigned iter = 0; iter < sim_params.packets_count; iter++ )
    {
/*
#ifdef LOGGING
        ...
        char cc;
        int k = 1;      // 00000001 Используемая маска
        //int kk = 128;   // 10000000
        int chet = 0;
        int and1;
        int ii = 8;
        i = 0;
        //static int64_t file_size = getFileSize(FILE_PATH); // Устаревшая функция
...
*/
        ...
        *num_of_samples2 = 7360;
/*        printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!! num_of_samples2 = %d \n", *num_of_samples2);
        ...
        for (i = 0; i < *num_of_samples2 * 2; i++)
        {
            //fprintf(filets, "%2.2f ", time_signal[i]); //старая версия вывода
            filets << time_signal[i] << "\n"; //timesignal
            Mybuff[i] = time_signal[i]; //timesignal
        }
...
/*
    ...
    printf("\nMybuff\n");
    //for (int i = 300; i < 500; i++) {   printf(" %f; ", Mybuff[i]);     }*/
    return Mybuff;
}
...
//    sprintf( instant_log_filename, "__I_log%f.log", SNR );
    ...
    printf("    Work MyReceiver...\n *Sizebuff = %d", sizebuff3);
    //for (int i = 0; i < sizebuff3; i++) {     printf(" %f; ", fxp_time_signal[i]);    }
