...
bool ETSI_RRS_TxRxChainControlServices::tx_from_buff(std::vector<std::complex<int16_t>> buff, size_t samps_per_buff) {
    ...
    if (usrpDevice->tx_stream->send(&buff.front(), samps_per_buff, usrpDevice->tx_md) != samps_per_buff) {
        cout << "\e[31m" << "Bytes was ERROR" << "\e[0m" << endl;
        return false;
    }
    else {
...
std::vector<std::complex<int16_t>> ETSI_RRS_TxRxChainControlServices::rx_to_buff(size_t samps_per_buff, float settling_time, int num_requested_samps) {
    ...
    std::vector<std::complex<int16_t>> buff(usrpDevice->rx_samps_per_buff);
